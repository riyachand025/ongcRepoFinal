    ...
    try {
        ...
        if (!process.env.EMAIL_USER) {
            ...
            return res.status(500).json({
                success: false,
                message: 'Email configuration not found. Please configure EMAIL_USER in environment variables.'
            });
        }
        ...
        if (attachTemplate) {
            ...
            if (pdfBuffer) {
                ...
                mailOptions.attachments.push({
                    filename: 'ONGC_Internship_Application_Form_Filled.pdf',
                    content: Buffer.from(pdfBuffer).toString("base64"), // convert Uint8Array → base64
                    type: "application/pdf",   // ✅ use `type` not `contentType`
        ...
        if (process.env.SIMULATE_EMAIL === 'true' || process.env.EMAIL_USER === 'demo@example.com') {
            ...
            let [response] = await sgMail.send(mailOptions);
